...
Of note, `Dockerfile.v2` was an attempt to use `bioconda` to simplify the container generation process, but was later abandoned due to packaging conflicts.
**Do not use** `Dockerfile.v2`.
...
apptainer shell linken.sif

# Run program that's available within linken.sif
apptainer exec linken.sif samtools
```

The `.sif` file extension is the convention.
It is not required for the container to properly be executed.
The final size of this container is around 770 MB.
...
The `linken.sif` is an executable.
Instead of issuing `apptainer shell linken.sif`, it can instead be run as follows: `./linken.sif`.
Place it under path (e.g. at `~/.local/bin/linkenc`) and now it can be called as easy as `linkenc` (*linken container*).

Depending on the situation, `build` can fail because of insufficient space in `/tmp`.
...
## Eevee Companion Container

To build, `cd` into the `companion` folder and run:

```bash
docker build --file eeveec.Dockerfile --tag aixnr/eeveec .
```

The `aixnr/eeveec` container image is shipped with `ipython` and its `CMD` directive was set to run it upon container execution.

```bash
docker run -it --rm aixnr/eeveec
```
